...
module Make (R : Rationals) (V : Value with type r = R.t) :
  ...
  let floor r =
    if V.is_int r.v then
      if is_pure_rational r then r else of_r (V.sub r.v V.one)
    else of_r (V.floor r.v)

  let ceiling r =
    if V.is_int r.v then
      if is_pure_rational r then r else of_r (V.add r.v V.one)
    else of_r (V.ceiling r.v)

  let print_offset fmt off =
    let c = R.compare off R.zero in
    if c = 0 then ()
    else if c > 0 then Format.fprintf fmt "+%aƐ" R.print off
    else Format.fprintf fmt "%aƐ" R.print off

  let print fmt r = Format.fprintf fmt "%a%a" V.print r.v print_offset r.offset
end
