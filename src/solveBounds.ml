...
module Make (Core : CoreSig.S) : S with module Core = Core = struct
  ...
  let look_for_next_pivot si pi non_basic =
    ...
    let is_lower =
      match status with ValueOK -> assert false | LowerKO -> 1 | UpperKO -> -1
    in
    try
      P.iter
        (fun x coef ->
...
*)
    ...
    if SX.is_empty env.fixme then { env with status = SAT }
      ...
      | Some (x, c, xi, use_x) ->
          Logs.debug ~src (fun p ->
              p ~header:"[solve_rec]" "pivot basic %a and non-basic %a@."
          ...
          let fixme =
            (* do this earlier to detect bad pivots *)
            if xi.vstatus == ValueOK then fixme else SX.add x fixme
          in
          let non_basic =
            P.fold
              (fun y _ non_basic ->
                let yi, use_y =
                  try MX.find y non_basic with Not_found -> assert false
                in
          ...
          let basic, non_basic, fixme =
            SX.fold
              (fun t (basic, non_basic, fixme) ->
                let ti0, r =
                  try MX.find t basic with Not_found -> assert false
                in
              ...
              *)
                ...
                let fixme =
                  if ti.vstatus == ValueOK then
                    if ti0.vstatus == ValueOK then fixme else SX.remove t fixme
                  else SX.add t fixme
                in
    ...
    try
      P.iter
        (fun x c ->
          ...
          let sg = R.sign c in
          if
            (sg > 0 && not (equals_optimum xi.value xi.maxi))
            || (sg < 0 && not (equals_optimum xi.value xi.mini))
          then (
  ...
  type 'a maximiza_basic = Free | Stuck | Progress of 'a

  let basic_var_to_pivot_for_maximization =
    let choose_best_pivot acc s si p c_px bnd_opt is_min =
      match bnd_opt with
      | None -> if !acc = Stuck then acc := Free (* !!! to check *)
      | Some { bvalue = bnd; _ } ->
          let tmp =
            if is_min then R2.sub si.value bnd else R2.sub bnd si.value
          in
          let ratio = R2.div_by_const (R.abs c_px) tmp in
          (match !acc with
          | Free | Stuck -> acc := Progress (ratio, s, si, p, c_px, bnd, is_min)
          | Progress (r_old, _, _, _, _, _, _) ->
              if R2.compare r_old ratio > 0 then
                acc := Progress (ratio, s, si, p, c_px, bnd, is_min));
          if R2.is_zero ratio then
            raise Exit (* in the case, the pivot is found*)
    in
  ...
  let update_valuation_without_pivot ({ basic; non_basic; _ } as env) x use_x
      new_xi diff _should_incr =
  ...
  let rec maximize_rec env opt rnd =
    Logs.debug ~src (fun p ->
        p "[maximize_rec] round %d // OPT = %a@." rnd P.print opt);
    Core.debug
      (Format.sprintf "[maximize_rec] round %d" rnd)
      env (Result.get None);
    Core.check_invariants env (Result.get None);
    match non_basic_to_maximize env opt with
    | None ->
        Logs.debug (fun p -> p "max reached@.");
        (rnd, env, Some (opt, true)) (* max reached *)
    | Some (_x, _c, _xi, _use_x, _should_incr) -> (
        Logs.debug (fun p -> p "pivot non basic var %a ?@." Var.print _x);
        match
          basic_var_to_pivot_for_maximization env _x _use_x _should_incr
        with
        | Free -> (
            Logs.debug (fun p ->
                p "non basic %a not constrained by basic vars: Set it to max@."
                  Var.print _x);
            match can_fix_valuation_without_pivot _should_incr _xi None with
            | Some (new_xi, diff) ->
                Logs.debug (fun p ->
                    p "No --> I can set value of %a to min/max WO pivot@."
                      Var.print _x);
                let env, opt =
                  ( update_valuation_without_pivot env _x _use_x new_xi diff
                      _should_incr,
                    opt )
                in
                (* no pivot *)
            ...
            | None ->
                Logs.debug (fun p ->
                    p "no pivot finally(no upper bnd), pb unbounded@.");
        ...
        | Progress (ratio, s, si, p, c_px, bnd, _is_min) ->
            ...
            let env, opt =
              match
                can_fix_valuation_without_pivot _should_incr _xi (Some ratio)
              with
              | Some (new_xi, diff) ->
                  Logs.debug (fun p ->
                      p "No --> I can set value of %a to min/max WO pivot@."
              ...
              | None ->
                  ...
                  let xi = _xi in
                  Logs.debug (fun p ->
                      p "[maximize_rec] pivot basic %a and non-basic %a@."
                        Var.print s Var.print x);
                  let basic = MX.remove s env.basic in
                  let non_basic = MX.remove x env.non_basic in
                  let q = gauss_pivot s p x c in
                  ...
                  let basic, non_basic =
                    SX.fold
                      (fun t (basic, non_basic) ->
                        ...
                        in
                        let cx =
                          try P.find x r with Not_found -> assert false
                        in
                        (*should update_ti*)
                        let diff_cx = R2.mult_by_const cx diff_xi_val in
                        let ti =
                          { ti0 with value = R2.add ti0.value diff_cx }
                        in
                        let ti = ajust_status_of_basic ti in
                        let r', changed = P.subst x q r in
  ...
  let maximize env opt0 =
    ...
    | SAT ->
        Logs.debug (fun p ->
            p "[maximize] pb SAT! try to maximize %a@." P.print opt0);
        ...
        if !unbnd then (env, Some (opt, false)) (* unbounded *)
          ...
          Logs.debug (fun p ->
              p "[maximize] pb SAT! Max found ? %b for %a == %a@."
