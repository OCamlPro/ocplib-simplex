...
module Make (Core : CoreSig.S) : S with module Core = Core = struct
  ...
  let get_unsat_core s env =
    try
      ...
      if not (consistent_bounds si) then
        Ex.union (Core.expl_of_min_bound si) (Core.expl_of_max_bound si)
      else
        match si.vstatus with
        | ValueOK -> assert false
        | LowerKO ->
            explain_poly env.non_basic p (Core.expl_of_min_bound si) true
        | UpperKO ->
            explain_poly env.non_basic p (Core.expl_of_max_bound si) false
    with Not_found ->
  ...
  let eval_eps (eps : V.t) (inf : R2.t) (sup : R2.t) =
  ...
  let eps_for_mini (i : Core.var_info) (min : R2.t) (eps : V.t) : V.t =
    assert (R2.is_pure_rational min || R.equal min.R2.offset R.one);
    let eps = eval_eps eps min i.value in
    eps

  let eps_for_maxi (i : Core.var_info) (max : R2.t) (eps : V.t) : V.t =
      ...
      MX.fold
        (fun _ (i, _) eps ->
          let eps' =
            ...
            | None, None -> eps
            | Some min, None -> eps_for_mini i min.bvalue eps
            | None, Some max -> eps_for_maxi i max.bvalue eps
            | Some min, Some max ->
                eps |> eps_for_mini i min.bvalue |> eps_for_maxi i max.bvalue
          in
    ...
    let max_v : Core.bound =
      Core.P.fold
        (fun x c (max_v : Core.bound) ->
          ...
          in
          let bnd = if R.sign c > 0 then xi.maxi else xi.mini in
          let ex =
            match bnd with
            | None -> Core.Ex.empty
  ...
  let get opt env =
    ...
    | SAT -> (
        ...
        | Some (p, true) ->
            Max (lazy (get_max_info env p), lazy (get_solution env)))
end
